...
class FakeDetector:
    ...
    na = 3
    nc = 2
    stride = torch.tensor([8.0, 16.0, 32.0, 64.0])
    anchors = torch.tensor(
        [
            [[2.375, 3.375], [5.5, 5.0], [4.75, 11.75]],
            [[6.0, 4.25], [5.375, 9.5], [11.25, 8.5625]],
            [[4.375, 9.40625], [9.46875, 8.25], [7.4375, 16.9375]],
            [[6.8125, 9.60938], [11.54688, 5.9375], [14.45312, 12.375]],
        ]
    )
...
def test_loss(
    batch, ota_loss, aux_loss, expected_loss, expected_loss_items, batch_loss_inputs
):
    model = FakeModel()
    ...
    model_outputs = [
        o.detach().clone().requires_grad_()
        for o in batch_loss_inputs[batch]["model_outputs"]
    ]
    ...
    assert (
        (loss_items.round(decimals=4) == expected_loss_items.round(decimals=4))
        .all()
        .item()
    )
